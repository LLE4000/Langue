...
- **Ergonomie, principes tenus dans toute l’application** : une seule grande action par écran (l’accueil montre la prochaine leçon, l’essentiel du jour sur une ligne, le reste replié sous « Plus ») ; onboarding en trois écrans (les niveaux sont repliés derrière « J’ai déjà des bases ») ; réglages en trois onglets courts (Profil, Voix, Exercices) qui ne gardent que les choix utiles, le dépannage de la voix étant replié ; le bouton « retour » du téléphone ferme les feuilles au lieu de quitter la page ; chaque feuille se termine par « Fermer » ; les pertes (abandon d’une partie, remplacement d’une sauvegarde, suppression) demandent confirmation ; les listes cliquables ne contiennent plus de bouton dans un bouton ; les boutons audio pulsent pendant la lecture ; les cartes se notent aussi d’un balayage (droite = Bien, gauche = Encore) ; les jeux à plusieurs n’écrivent pas dans la mémoire de révision du profil (`record={false}`).
