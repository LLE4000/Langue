...
export function Profile() {
  const t = T();
  usePage(t.profile.title, { back: '/', thai: { th: 'โปรไฟล์', rom: 'proo-fai' } });
  ...
  const Row = ({ to, ico, t: title, s }: { to: string; ico: string; t: string; s: string }) => <Link className="row" to={to}><Ico name={ico} /><span className="mid"><span className="t">{title}</span><span className="s">{s}</span></span><span className="end"><span className="chev">›</span></span></Link>;
  return (
    <>
