...
export function Classifiers() {
  ...
  return (
    <>
      ...
      <div className="btns" style={{ margin: '14px 0' }}><Link className="btn soft sm" to="/train/flashcards">🗂️ Cartes</Link><Link className="btn soft sm" to="/train/match">🔗 Associer</Link></div>
      <div className="h2">Les plus courants</div>
      <div className="list">{CLF_ITEMS.map((c, i) => <ItemRow key={c.id} it={c} onClick={() => setDetail(i)} />)}</div>
      <div className="note" style={{ marginTop: 12 }}>En cas de doute, <Thai text="อัน" /> <Rom text="an" /> dépanne pour les objets, et pointer du doigt en disant <Thai text="อันนี้" /> <Rom text="an-níi" /> (« celui-ci ») fonctionne toujours.</div>
