...
import { AudioPair, Icon, Ico, Sheet, Thai, Rom } from '@/components/ui';
...
function Row({ ico, t, s, to }: { ico: string; t: string; s: string; to: string }) {
  return <Link className="row" to={to}><Ico name={ico} /><span className="mid"><span className="t">{t}</span><span className="s">{s}</span></span><span className="end"><span className="chev">›</span></span></Link>;
}
...
function Five() {
  ...
  return <><TheoryBlockView b={{ kind: 'tones' }} /><p className="foot-note">Les noms thaïs des tons : {TONES.map((t, i) => <span key={t.id}>{i > 0 ? ' · ' : ''}<Thai text={t.thaiName} /> <Rom text={t.rom} /></span>)}</p></>;
}
...
function SetPlayer() {
  ...
  return (
    <>
      <div className="stage" onClick={() => sp.speak(w.say)} style={{ cursor: 'pointer' }}><span className="tag" style={{ color: tn.color }}>ton {L(tn.name)}</span><div className="big s1" style={{ fontSize: 'clamp(76px, 28vw, 124px)' }}><Thai text={w.thai} /></div><div className="rom reveal" style={{ color: tn.color }}>{w.rom}</div><div className="mut">{L(w.meaning)}</div><ToneCurve tone={w.tone} /></div>
      <div className="audio"><AudioPair text={w.say} big /><button className="ib big" onClick={() => setDetail(true)} aria-label="Pourquoi ce ton ?"><Icon name="bulb" /></button></div>
      ...
      <div className="btns mt-3"><button className="btn soft sm auto" onClick={() => nav(`/explore/tones/set/${(idx - 1 + th.TONE_SETS.length) % th.TONE_SETS.length}`)} aria-label="Série précédente"><Icon name="back" size={18} /></button><button className="btn sm" onClick={playAll}><Icon name={playing ? 'stop' : 'play'} size={16} /> {playing ? 'Arrêter' : 'Écouter toute la série'}</button><button className="btn soft sm auto" onClick={() => nav(`/explore/tones/set/${(idx + 1) % th.TONE_SETS.length}`)} aria-label="Série suivante"><Icon name="next" size={18} /></button></div>
...
function Table() {
  ...
  const head = <tr><th /><th><span className="c-M">moyenne</span></th><th><span className="c-H">haute</span></th><th><span className="c-L">basse</span></th></tr>;
