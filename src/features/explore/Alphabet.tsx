  ...
  return (
    <div className="peek" role="region" aria-label="Aperçu de la lettre">
      ...
      <div className="peek-acts">
        ...
        {it.kind === 'cons' && <button className="ib" onClick={() => sp.speak(sound)} aria-label="Le son seul" title="Le son seul"><span lang="th" className="th th-s">{sound}</span></button>}
    ...
    <>
      ...
      <details className="fold">
        ...
        {th.LOOKALIKES.map((g, k) => {
          ...
          return (
            <div key={k} className="lgrid c5 mb-3">
              {g.map((x) => { const c = CONS_ITEMS.find((it) => it.thai === x)!; return <LetterCell key={x} it={c} active={peekItem?.id === c.id} onClick={() => tap(c, gi)} />; })}
              <Link className="cell jade" to={`/explore/listen?ids=${encodeURIComponent(gi.join(','))}`} aria-label="Écouter ce groupe en boucle"><Icon name="repeat" /></Link>
            </div>
          );
        })}
      </details>
