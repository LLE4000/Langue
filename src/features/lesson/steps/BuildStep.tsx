...
import { T } from '@/i18n';
import { AudioButton, Fr, Icon, Thai, Rom } from '@/components/ui';
...
export function BuildStep({ step, onDone }: { step: RuntimeStep & { type: 'build' }; onDone: (r: StepResult) => void }) {
  ...
  const inLesson = useStepProgress((i + (res !== null ? 1 : 0)) / Math.max(1, items.length));
  if (!cur) return null;
  ...
  return (
    <>
      ...
      <div className="stage compact"><div className="frbig fr-m"><Fr text={cur.sent.tr} /></div></div>
      <div className="audio"><AudioButton text={full} /><span className="xs mut">indice audio</span></div>
      <div className="tline" aria-label="Votre phrase">{pick.length === 0 && <span className="ph">Votre phrase apparaît ici · touchez un mot placé pour le retirer</span>}{pick.map((k, j) => <button key={j} className="tok" disabled={res !== null} onClick={() => setPick(pick.filter((_, x) => x !== j))} aria-label={`Retirer ${cur.toks[k].thai}`}><Fr text={cur.toks[k].thai} /></button>)}</div>
      <div className="tline bank">{cur.order.map((k) => <button key={k} className={`tok ${pick.includes(k) ? 'used' : ''}`} disabled={res !== null} onClick={() => setPick([...pick, k])}><Fr text={cur.toks[k].thai} /></button>)}</div>
      <div className="sp" />
      <StepFooter tone={res === null ? '' : res ? 'ok' : 'ko'} meta={res === null ? <><span>Touchez les mots dans l’ordre, puis vérifiez</span>{!inLesson && <span className="b">{i + 1} / {items.length}</span>}</> : undefined}>
        {res === null ? (
          <div className="btns">
            {pick.length > 0 && <button className="btn ghost auto" onClick={() => setPick([])}>Effacer</button>}
